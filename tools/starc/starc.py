...
def generate_header(schema: Schema) -> str:
    ...
    out.append('#include "core_sysdb/WalTypes.h"')
...
def generate_codec(schema: Schema) -> str:
    ...
    # applyFieldWrite
    out.append("bool applyFieldWrite(SystemState& state, ComponentId comp, uint8_t field_tag, const uint8_t* val, uint8_t len) {")
    out.append("    switch (comp) {")
    for comp in schema.components:
        out.append(f"        case ComponentId::{comp.name.upper()}: {{")
        out.append("            uint8_t tag = 0;")
        out.append("            #define X(type, name, def, bit, access) \\")
        out.append(f"                if (tag++ == field_tag) return deserializeAny(state.{comp.name.lower()}.name, val, len);")
        out.append("            #define X_STR(name, size, def, bit, access) \\")
        out.append(f"                if (tag++ == field_tag) return deserializeString(state.{comp.name.lower()}.name, size, val, len);")
        out.append("            #define X_COLOR(name, def, bit, access) \\")
        out.append(f"                if (tag++ == field_tag) return deserializeRgbColor(state.{comp.name.lower()}.name, val, len);")
        out.append(f"            {comp.name.upper()}_FIELDS")
        out.append("            #undef X")
        out.append("            #undef X_STR")
        out.append("            #undef X_COLOR")
    ...
    # diffAndEmitWal
    out.append("ComponentMask diffAndEmitWal(const SystemState& old_s, const SystemState& new_s, WalRingBuffer& wal) {")
    ...
    for comp in schema.components:
        ...
        out.append("        uint8_t tag = 0;")
        out.append("        #define X(type, name, def, bit, access) \\")
        out.append(f"            if (old_s.{comp.name.lower()}.name != new_s.{comp.name.lower()}.name) {{ \\")
        out.append(f"                if (bit != 0) changed |= (COMP::{comp.name.upper()} | bit); \\")
        out.append(f"                else changed |= COMP::{comp.name.upper()}; \\")
        out.append("                uint8_t buf[sizeof(type) > 160 ? sizeof(type) : 160]; \\")
        out.append(f"                uint8_t len = serializeAny(new_s.{comp.name.lower()}.name, buf, sizeof(buf)); \\")
        out.append(f"                wal.push(static_cast<uint8_t>(ComponentId::{comp.name.upper()}), tag, buf, len); \\")
        out.append("            } \\")
        out.append("            tag++;")
        out.append("")
        out.append("        #define X_STR(name, size, def, bit, access) \\")
        out.append(f"            if (std::strncmp(old_s.{comp.name.lower()}.name, new_s.{comp.name.lower()}.name, size) != 0) {{ \\")
        out.append(f"                if (bit != 0) changed |= (COMP::{comp.name.upper()} | bit); \\")
        out.append(f"                else changed |= COMP::{comp.name.upper()}; \\")
        out.append("                uint8_t buf[size]; \\")
        out.append(f"                uint8_t len = serializeString(new_s.{comp.name.lower()}.name, size, buf, sizeof(buf)); \\")
        out.append(f"                wal.push(static_cast<uint8_t>(ComponentId::{comp.name.upper()}), tag, buf, len); \\")
        out.append("            } \\")
        out.append("            tag++;")
        out.append("")
        out.append("        #define X_COLOR(name, def, bit, access) \\")
        out.append(f"            if (old_s.{comp.name.lower()}.name.r != new_s.{comp.name.lower()}.name.r || \\")
        out.append(f"                old_s.{comp.name.lower()}.name.g != new_s.{comp.name.lower()}.name.g || \\")
        out.append(f"                old_s.{comp.name.lower()}.name.b != new_s.{comp.name.lower()}.name.b) {{ \\")
        out.append(f"                if (bit != 0) changed |= (COMP::{comp.name.upper()} | bit); \\")
        out.append(f"                else changed |= COMP::{comp.name.upper()}; \\")
        out.append("                uint8_t buf[3]; \\")
        out.append(f"                uint8_t len = serializeRgbColor(new_s.{comp.name.lower()}.name, buf, sizeof(buf)); \\")
        out.append(f"                wal.push(static_cast<uint8_t>(ComponentId::{comp.name.upper()}), tag, buf, len); \\")
        out.append("            } \\")
        out.append("            tag++;")
        out.append(f"        {comp.name.upper()}_FIELDS")
    ...
    # serializeSnapshot
    out.append("void serializeSnapshot(const SystemState& state, uint32_t head_seq, std::vector<WalRecordEntry>& out_snapshot) {")
    out.append("    out_snapshot.clear();")
    out.append("")
    out.append("    auto add_entry = [&](ComponentId comp, uint8_t tag, const uint8_t* val, uint8_t len) {")
    out.append("        WalRecordEntry e;")
    out.append("        e.seq = head_seq;")
    out.append("        e.component_id = static_cast<uint8_t>(comp);")
    out.append("        e.field_tag = tag;")
    out.append("        e.value.assign(val, val + len);")
    out.append("        out_snapshot.push_back(std::move(e));")
    out.append("    };")
    out.append("")
    for comp in schema.components:
        out.append(f"    // ── Snapshot Component: {comp.name.upper()} ─────────────────────────────")
        out.append("    {")
        out.append("        uint8_t tag = 0;")
        out.append("        #define X(type, name, def, bit, access) { \\")
        out.append(f"            uint8_t buf[sizeof(type) > 160 ? sizeof(type) : 160]; \\")
        out.append(f"            uint8_t len = serializeAny(state.{comp.name.lower()}.name, buf, sizeof(buf)); \\")
        out.append(f"            add_entry(ComponentId::{comp.name.upper()}, tag++, buf, len); \\")
        out.append("        }")
        out.append("")
        out.append("        #define X_STR(name, size, def, bit, access) { \\")
        out.append(f"            uint8_t buf[size]; \\")
        out.append(f"            uint8_t len = serializeString(state.{comp.name.lower()}.name, size, buf, sizeof(buf)); \\")
        out.append(f"            add_entry(ComponentId::{comp.name.upper()}, tag++, buf, len); \\")
        out.append("        }")
        out.append("")
        out.append("        #define X_COLOR(name, def, bit, access) { \\")
        out.append("            uint8_t buf[3]; \\")
        out.append(f"            uint8_t len = serializeRgbColor(state.{comp.name.lower()}.name, buf, sizeof(buf)); \\")
        out.append(f"            add_entry(ComponentId::{comp.name.upper()}, tag++, buf, len); \\")
        out.append("        }")
        out.append(f"        {comp.name.upper()}_FIELDS")
        out.append("        #undef X")
