...
esp_err_t IoExpander::init(i2c_master_bus_handle_t bus_handle, uint8_t address) {
  // Create the high-level expander handle (used for set_dir / set_level)
  esp_err_t ret = esp_io_expander_new_i2c_tca95xx_16bit(
      bus_handle, address, &m_expander);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to create TCA9555 expander: %s", esp_err_to_name(ret));
    return ret;
  }
