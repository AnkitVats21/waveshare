...
bool WifiManager::begin() {
  ...
  // 2. Network interface infrastructure + default system event loop
  ...
  // 3. Configure and initialize the WiFi driver
  ...
  // 5. Build station config and start hardware
  ...
  // Disable power-save mode for low-latency real-time audio streaming
...
void WifiManager::sysEventHandler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    if (instance->m_retry_cnt < instance->m_config.max_retry) {
      ...
      LOGI_WIFI("Retrying connection to AP... (%d/%d)",
                instance->m_retry_cnt, instance->m_config.max_retry);
    } else {
